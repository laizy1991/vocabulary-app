...
function getTodayStats() {
  ...
  let stmt = db.prepare('SELECT COUNT(*) as count FROM vocabulary WHERE created_at >= ?');
  stmt.bind([todayStart]);
  const todayAdded = stmt.step() ? stmt.getAsObject().count : 0;
  stmt.free();
  
  stmt = db.prepare('SELECT COUNT(*) as count FROM vocabulary WHERE last_reviewed_at >= ?');
  stmt.bind([todayStart]);
  const todayReviewed = stmt.step() ? stmt.getAsObject().count : 0;
  stmt.free();
  
  return { todayAdded, todayReviewed };
}
...
app.post('/api/vocabulary', (req, res) => {
  try {
    ...
    // 检查是否已存在相同词汇（不区分大小写）
    const checkStmt = db.prepare('SELECT * FROM vocabulary WHERE LOWER(word) = LOWER(?)');
    checkStmt.bind([word]);
    let existing = null;
    ...
    if (existing) {
      ...
      const id = uuidv4();
      db.run(`
        INSERT INTO vocabulary (id, word, pinyin, meaning, english_meaning, detail, phrases, sentences, level, category, status, language, review_count, correct_count, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [id, word, pinyin || null, meaning || null, englishMeaning || null, detail || null, 
...
app.post('/api/exercises/generate', async (req, res) => {
  try {
    ...
    if (vocabIds && vocabIds.length > 0) {
      ...
      const stmt = db.prepare(`SELECT * FROM vocabulary WHERE id IN (${placeholders}) ORDER BY RANDOM() LIMIT ?`);
      stmt.bind([...vocabIds, count]);
      while (stmt.step()) vocabList.push(stmt.getAsObject());
      stmt.free();
    } else {
      const stmt = db.prepare('SELECT * FROM vocabulary WHERE meaning IS NOT NULL ORDER BY RANDOM() LIMIT ?');
      stmt.bind([count]);
      while (stmt.step()) vocabList.push(stmt.getAsObject());
      stmt.free();
    }
    ...
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [exerciseId, type, '词汇练习', JSON.stringify(questions), JSON.stringify(questions.map(q => q.answer)),
...
app.post('/api/exercises/:id/submit', (req, res) => {
  try {
    ...
    vocabIds.forEach(vid => {
      try {
        db.run('UPDATE vocabulary SET review_count = review_count + 1, correct_count = correct_count + ?, last_reviewed_at = ? WHERE id = ?',
          [1, now, vid]);
      } catch (e) {}
    });
    saveDatabase();
...
// 标记错题为已掌握
app.put('/api/wrong-answers/:id/solve', (req, res) => {
  try {
    const { id } = req.params;
    const now = new Date().toISOString();
    
    const checkStmt = db.prepare('SELECT * FROM wrong_answers WHERE id = ?');
    checkStmt.bind([id]);
    let existing = null;
    if (checkStmt.step()) existing = checkStmt.getAsObject();
    checkStmt.free();
...
app.delete('/api/wrong-answers/:id', (req, res) => {
  try {
    ...
    db.run('DELETE FROM wrong_answers WHERE id = ?', [id]);
    saveDatabase();
...
app.get('/api/stats', (req, res) => {
  try {
    ...
    // 获取错题本统计
    let wrongStmt = db.prepare('SELECT COUNT(*) as count FROM wrong_answers WHERE solved = 0');
    const unsolvedCount = wrongStmt.step() ? wrongStmt.getAsObject().count : 0;
    wrongStmt.free();
    ...
    res.json({
      ...
      data: { 
        total, 
        todayAdded: todayStats.todayAdded, 
        todayReviewed: todayStats.todayReviewed,
...
async function startServer() {
  await initDatabase();
  app.listen(PORT, () => {
    console.log(`词汇管理助手 v3 后端服务运行在 http://localhost:${PORT}`);
